...
Pipeline position:
  Stage 5 — Step 2 Export (Close Reading Corpus)
  Reads from: corpus_view, pages, platform_term_counts, keyness_results,
              step2_sample (optional — written by 03_step1_topics.py)
  Produces:   output/step_2/  (plain-text .txt files + README.txt)
  Follows:    02_step1_analysis.py  (keyness_results, platform_term_counts)
              03_step1_topics.py    (step2_sample)

What this script does:
  Step 1 (02_step1_analysis.py, 03_step1_topics.py) identified WHAT is
  statistically distinctive about B2B vs B2W language — which terms are
  disproportionately associated with each audience, how co-occurrence
  profiles differ, and how the topic space divides.
  ...
  Export D — Step 2 theory-guided sample (from step2_sample table)
    Pages pre-selected by 03_step1_topics.py: the highest-PMI collocate
    pages per hypothesis cluster term, drawn from the LDA topic assignments
    most relevant to each H1–H5 hypothesis.  This provides a theoretically
    focused subset for deep reading on each hypothesis.
    Skipped gracefully if step2_sample is not yet populated.
...
Output structure:
    output/step_2/
      ...
      D_theory_sample/
        H1a_visibility_client.txt / H1a_visibility_worker.txt
        H1b_automation_client.txt  / ...
        H2_flexibility_worker.txt  / ...
        H3_resource_worker.txt     / ...
        H4_alienation_worker.txt   / ...
        H5_coloniality_worker.txt  / ...
...
Text handling:
  ...
  - find_kwic() uses a simple regex sentence splitter — adequate for close
    reading; not a full NLP pipeline.
...
Prerequisites:
  Run in order before this script:
    01_prepare_corpus.py   — populates corpus_view
    02_step1_analysis.py   — populates platform_term_counts, keyness_results
    03_step1_topics.py     — populates step2_sample (required for Export D)

  The script raises RuntimeError immediately if corpus_view,
  platform_term_counts, or keyness_results is missing.
  Export D is skipped (with a logged warning) if step2_sample is absent.

Usage:
    python3 src2/05_step2_export.py
"""
...
# ---------------------------------------------------------------------------
# Terms — organised by hypothesis and audience register.
# These are the terms Step 1 identified as theoretically meaningful after
# artifact filtering.  Edit this list as your close reading evolves.
# ---------------------------------------------------------------------------
...
WORKER_TERMS = {
    # H1a — Labour visibility gap: labour terms suppressed in B2B,
    #        foregrounded in B2W
    "worker":       "H1a",
    "work":         "H1a",
    "job":          "H1a",
    ...
    "remote":       "H1a",
    "project":      "H1a",
    # H2 — Flexibility framing: B2W frames gig work as autonomy
    "flexible":     "H2",
    "freedom":      "H2",
    "schedule":     "H2",
    "anytime":      "H2",
    # H3 — Resource framing: B2W frames labour as income opportunity
    "income":       "H3",
    "opportunity":  "H3",
    "skill":        "H3",
    # H4 — Alienation / community rhetoric
    "community":    "H4",
    "connect":      "H4",
    # H5 — Coloniality: global reach framing in B2W
    "global":       "H5",
}
...
# Must match domains as stored in corpus_view.domain
# (get_pages_for_domain handles www. / suffix variants automatically)
PAIRS = [
...
def has_table(conn, name: str) -> bool:
    """Return True if table or view *name* exists in the database."""
...
def get_page_text(conn, page_id):
    ...
    This is the text originally tokenised for Step 1 analysis; returning
    to it allows Step 2 to read in full context.
...
def get_top_pages_for_term(conn, term, audience, n):
    ...
    Queries platform_term_counts (written by 02_step1_analysis.py) joined to
    corpus_view for metadata.  Relative frequency (rel_freq) is the term's
    count per 1000 tokens for that domain, normalising for domain size.
...
def get_all_pages_for_audience(conn, audience):
    ...
    Used by Export B (KWIC) which needs to scan every page in the sub-corpus
    for a complete distributional picture of how each term is used.
...
def get_pages_for_domain(conn, domain):
    ...
    Returns:
        List of dicts with keys: page_id, url, platform_name, domain,
        audience.  Empty list if domain not found after all strategies.
    """
    # 1. Exact match
    rows = conn.execute("""
    ...
    if rows:
        log.info(
            f"  '{domain}' matched via substring '{stem}' to '{rows[0]['domain']}'"
        )
        return [dict(r) for r in rows]
    ...
    log.warning(
        f"  No pages found for '{domain}' (tried exact, '{alt}', stem '{stem}')"
    )
...
def split_sentences(text):
    ...
    Splits on .!? followed by whitespace and a capital letter.  Intentionally
    simple — a missed split produces slightly wider context windows but does
    not invalidate the concordance.
...
def find_kwic(text, term, context_n):
    ...
    Uses whole-word boundary matching (\\b) to avoid partial matches
    (e.g. "automate" should not match "automation" unless both are listed).

    Args:
        text      : Full page text.
        term      : Lemma string to search for (case-insensitive).
        context_n : Number of surrounding sentences to include on each side.
...
def highlight_term(sentence, term):
    ...
    ASCII markers are more legible than bold/italic in plain-text files and
    survive copy-paste into coding tools without formatting loss.
    ...
    Returns:
        Annotated string with every occurrence wrapped in >>>...<<<.
    """
    pattern = re.compile(r'\b(' + re.escape(term) + r')\b', re.IGNORECASE)
...
def truncate_text(text, max_chars=8000):
    ...
    Truncating keeps individual export files at a readable size.  Splitting
    on the last '\\n\\n' within the capped range ensures the text ends at a
    paragraph break rather than mid-sentence.
...
def write_page_block(f, meta, text, term=None):
    ...
    if "rel_freq" in meta:
        f.write(
            f"Term freq: {meta['rel_freq']:.2f}\u2030  (abs: {meta.get('freq', '?')})\n"
        )
    f.write(f"{'—' * 60}\n\n")
...
def write_kwic_block(f, meta, kwic_hits, term):
    ...
    f.write(f"{'—' * 60}\n")
    f.write(
        f"Platform: {meta.get('platform_name', meta.get('domain', 'n/a'))}"
        f"  |  {meta.get('url', '')}\n"
    )
    f.write(f"{'—' * 60}\n")
...
def export_a_top_pages(conn, out_dir, generated_at):
    ...
    queries platform_term_counts to find domains where the term has the
    highest relative frequency, then writes those pages with reading-guide
    prompts.
    ...
    for audience, term_dict in [("client", CLIENT_TERMS), ("worker", WORKER_TERMS)]:
        ...
        for term, hypothesis in term_dict.items():
            ...
            with open(out_path, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"  Term '{term}' is {audience}-distinctive (Step 1 keyness).\n"
                )
                f.write(
                    f"  Question for Step 2: What is '{term}' doing rhetorically "
                    f"in {audience.upper()} texts?\n"
                )
                f.write(f"  Is its usage consistent with {hypothesis}?\n")
                f.write(
                    f"  Are there unexpected usages that complicate the hypothesis?\n\n"
                )
                for meta in pages:
...
def export_b_kwic(conn, out_dir, generated_at):
    """
    Export B: KWIC concordance — every occurrence of each term across the
    audience sub-corpus.

    Unlike Export A (top-N pages per term), Export B sweeps the ENTIRE
    audience sub-corpus and extracts every sentence containing the term,
    with ±CONTEXT_SENTENCES surrounding sentences.  This gives a complete
    distributional picture of how the term is used — essential for
    identifying recurring patterns and construction types.
    ...
    for audience, term_dict in [("client", CLIENT_TERMS), ("worker", WORKER_TERMS)]:
        ...
        for term, hypothesis in term_dict.items():
            ...
            with open(out_path, "w", encoding="utf-8") as f:
                ...
                f.write("READING NOTES:\n")
                f.write(
                    f"  Scan these concordance lines for recurring patterns.\n"
                )
                f.write(
                    f"  What subjects and objects appear with '{term}'?\n"
                )
                f.write(
                    f"  What verbs frame it?  What is the implied agent?\n"
                )
                f.write(
                    f"  Note any lines that surprise you or complicate {hypothesis}.\n\n"
                )
                f.write(f"{'=' * 72}\n\n")
...
def export_c_within_pair(conn, out_dir, generated_at):
    ...
    and one from the B2W domain.  Within-pair design controls for company-level
    style variation.  Register differences that persist within pairs are the
    cleanest evidence of audience-driven rhetorical adaptation.

    Sampling strategy: evenly-spaced stride sampling (every Nth page)
    rather than taking the first PAIR_SAMPLE_N pages, ensuring coverage
    across different site sections.
    ...
    for company_id, b2b_domain, b2w_domain in PAIRS:
        for domain, label in [(b2b_domain, "b2b"), (b2w_domain, "b2w")]:
            ...
            # Stride sampling — covers site sections rather than just first N
            ...
            counterpart  = "b2w" if label == "b2b" else "b2b"
            ...
            with open(out_path, "w", encoding="utf-8") as f:
                ...
                f.write("READING NOTES:\n")
                f.write(
                    f"  Read this alongside {company_id}_{counterpart}.txt\n"
                )
                f.write(
                    f"  This is the SAME COMPANY communicating to a DIFFERENT audience.\n"
                )
                f.write(f"  Question: What changes? What stays the same?\n")
                f.write(
                    f"  Pay attention to: labour vocabulary, compensation framing,\n"
                    f"  worker agency, automation claims, quality rhetoric.\n\n"
                )
                watch = list(CLIENT_TERMS.keys())[:5] + list(WORKER_TERMS.keys())[:5]
                f.write(f"  Terms to watch: {', '.join(watch)}\n\n")
...
# ---------------------------------------------------------------------------
# Export D: Theory-guided sample from step2_sample
# ---------------------------------------------------------------------------

def export_d_theory_sample(conn, out_dir, generated_at):
    """
    Export D: pages pre-selected by 03_step1_topics.py for deep theory reading.

    The step2_sample table stores the highest-PMI collocate pages per
    hypothesis cluster term, drawn from the LDA topic assignments most
    relevant to each H1–H5 hypothesis.  This provides a focused subset for
    deep reading on each hypothesis without requiring the analyst to browse
    the entire corpus.

    Skipped (with WARNING) if step2_sample does not exist — this occurs when
    03_step1_topics.py has not been run yet.

    Output: output/step_2/D_theory_sample/{hypothesis}_{audience}.txt

    Args:
        conn         : Open sqlite3.Connection.
        out_dir      : Path to output/step_2/ (parent of D_theory_sample/).
        generated_at : Timestamp string for file headers.
    """
    if not has_table(conn, "step2_sample"):
        log.warning(
            "Export D skipped — 'step2_sample' table not found. "
            "Run 03_step1_topics.py first."
        )
        return

    rows = conn.execute(
        "SELECT DISTINCT hypothesis, audience FROM step2_sample ORDER BY hypothesis, audience"
    ).fetchall()
    if not rows:
        log.warning("Export D skipped — step2_sample is empty.")
        return

    log.info("Export D — Theory-guided sample (step2_sample)")
    base = out_dir / "D_theory_sample"
    base.mkdir(parents=True, exist_ok=True)
    ...
    for row in rows:
        ...
        sample_rows = conn.execute("""
            ...
            JOIN corpus_view cv ON cv.page_id = s.page_id
            WHERE s.hypothesis = ? AND s.audience = ?
            ORDER BY s.pmi DESC
        """, [hyp, audience]).fetchall()

        if not sample_rows:
            continue

        out_path = base / f"{hyp}_{audience}.txt"
        with open(out_path, "w", encoding="utf-8") as f:
            write_header(
                f,
                f"EXPORT D — Theory-guided sample: {hyp}\n"
                f"Audience: {audience.upper()}  |  {len(sample_rows)} pages\n"
                f"Selected by: highest-PMI collocate pages for hypothesis cluster terms",
                generated_at,
            )
            f.write("READING NOTES:\n")
            f.write(
                f"  These pages were selected because they contain terms with\n"
                f"  the strongest associative signal for {hyp}.\n"
            )
            f.write(
                f"  They represent the corpus's most hypothesis-relevant instances.\n"
            )
            f.write(
                f"  Question: Does the rhetorical use confirm, nuance, or complicate "
                f"{hyp}?\n\n"
            )

            for sr in sample_rows:
                text, _ = get_page_text(conn, sr["page_id"])
                meta = {
                    "url":           sr["url"],
                    "platform_name": sr["platform_name"],
                    "platform_type": sr["platform_type"],
                    "audience":      audience,
                }
                # Surface which term triggered this page's inclusion
                f.write(f"  [Trigger term: '{sr['term']}'  PMI={sr['pmi']:.3f}]\n")
                write_page_block(f, meta, text, term=sr["term"])

        log.info(
            f"    {hyp}_{audience}.txt  ({len(sample_rows)} pages)"
        )
...
def write_readme(out_dir, generated_at):
    """
    Write README.txt to out_dir — a plain-text reading guide for the corpus.

    Explains the four export types, their intended use, and a structured
    ...
    with open(out_dir / "README.txt", "w", encoding="utf-8") as f:
        ...
        f.write("PURPOSE\n")
        f.write("-------\n")
        f.write("Step 1 identified statistically distinctive terms and topic clusters.\n")
        f.write("Step 2 asks: what are those terms and topics actually doing?\n")
        ...
        f.write("  toloka_b2b.txt / toloka_b2w.txt  etc.\n")
        f.write("  Same company, different audience. Read side by side.\n")
        f.write("  Register differences here are especially strong evidence.\n\n")

        f.write("D_theory_sample/\n")
        f.write("  <hypothesis>_<audience>.txt\n")
        f.write("  Pages with highest PMI for each hypothesis cluster term.\n")
        f.write("  Start here for deep reading on a specific hypothesis.\n\n")
        ...
        f.write("   - Map it to a hypothesis (H1a / H1b / H1c / H2–H5)\n\n")
        ...
        f.write("------------------------\n")
        f.write(
            "H1a Labour visibility:          Labour terms lower in B2B than B2W\n"
        )
        f.write(
            "H1b Automation myth:            B2B frames outputs as autonomous\n"
        )
        f.write(
            "H1c Strategic hypervisibility:  B2B foregrounds human as quality signal\n"
        )
        f.write(
            "H2  Flexibility framing:        B2W frames gig work as schedule freedom\n"
        )
        f.write(
            "H3  Resource framing:           B2W frames labour as income/skill opportunity\n"
        )
        f.write(
            "H4  Alienation / community:     B2W uses community rhetoric to mask isolation\n"
        )
        f.write(
            "H5  Coloniality:                B2W global reach framing — targets Global South\n"
        )
...
def main():
    """
    Entry point — runs all four exports and writes README.txt.
    ...
    missing.  Export D is skipped gracefully if step2_sample is absent.

    Pipeline prerequisites:
      01_prepare_corpus.py   (corpus_view)
      02_step1_analysis.py   (platform_term_counts, keyness_results)
      03_step1_topics.py     (step2_sample — required for Export D only)
    """
    if not Path(DB_PATH).exists():
        raise FileNotFoundError(f"Database not found: {DB_PATH}")

    log.info("=" * 60)
    log.info("05_step2_export.py — Step 2 Guided Close Reading Corpus")
    log.info("=" * 60)
    ...
    # Hard prerequisite check — abort if core tables are missing
    required = ["corpus_view", "platform_term_counts", "keyness_results"]
    for table in required:
        if not has_table(conn, table):
            raise RuntimeError(
                f"'{table}' not found — run 01_prepare_corpus.py and "
                f"02_step1_analysis.py before this script."
            )
    ...
    export_d_theory_sample(conn, OUTPUT_DIR, generated_at)
    write_readme(OUTPUT_DIR, generated_at)
