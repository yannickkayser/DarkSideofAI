...
def insert_batch(
    ...
    cursor.executemany("""
        INSERT OR REPLACE INTO pages_tfidf
            (page_id, url, unigrams, bigrams, token_count)
        VALUES (?, ?, ?, ?, ?)
    """, tfidf_rows)
    cursor.executemany("""
        INSERT OR REPLACE INTO pages_embedding
            (page_id, url, clean_text, tokenized_text)
        VALUES (?, ?, ?, ?)
    """, embed_rows)
    conn.commit()
...
def process(db_path: str, batch_size: int = BATCH_SIZE):
    ...
    # We scan in batches here too to avoid loading everything into memory.
    ...
    for batch_num, start in enumerate(range(0, total, batch_size), 1):
        ...
        try:
            ...
            log.warning(
                f"  Batch {batch_num}/{n_batches} — error during bigram scan, skipping:\n"
                + traceback.format_exc()
            )

    frequent_bigrams = {bg for bg, c in bigram_counts.items() if c >= MIN_BIGRAM_FREQ}
    log.info(f"  Keeping {len(frequent_bigrams)} bigrams with freq ≥ {MIN_BIGRAM_FREQ}")
    ...
    for batch_num, start in enumerate(range(0, total, batch_size), 1):
        ...
        log.info(f"  Batch {batch_num}/{n_batches}  (pages {start+1}–{batch_end})")
        ...
        try:
            ...
            log.error(
                f"  Batch {batch_num}/{n_batches} FAILED — skipping, will log IDs:\n"
                + traceback.format_exc()
            )
    ...
    # --- Summary ---
    log.info("=" * 60)
    log.info(f"Finished.  Inserted: {inserted_pages}/{total} pages.")

    if failed_batches:
        failed_id_count = sum(len(b["page_ids"]) for b in failed_batches)
        log.warning(f"{len(failed_batches)} batch(es) failed ({failed_id_count} pages skipped).")
